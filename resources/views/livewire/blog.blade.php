            ...
            <div class="row">
                <div class="col-8 mx-auto text-center">
                    ...
                    <ul class="list-inline breadcrumbs text-capitalize" style="font-weight:500">
                        ...
                        <li class="list-inline-item">/ &nbsp; <span>Blog</span>
                        </li>
                    </ul>
                </div>
            </div>
                        ...
                        <ul class="list-unstyled widget-list">
                            @if ($categories->isNotEmpty())
                                @foreach ($categories as $category)
                                    <li><a  href="{{ route('blogs') . '?categorySlug=' . $category->slug }}"
                                            wire:navigate>{{ $category->name }}
                                            <small class="ml-auto">({{ $category->articles_count ?? 0 }})</small>
                                        </a>
                                    </li>
                                @endforeach
                            @endif
                                ...
                                <a class="text-black" href="{{route('blog-detail',$latestarticle->id)}}" wire:navigate>
                                    <div class="widget-post-image flex-shrink-0 me-3">
                                        <img class="rounded" loading="lazy" decoding="async"
                                            src="{{asset('storage/'.$latestarticle->image)}}" alt="Post Thumbnail">
                                    </div>
                                </a>
                                <div class="flex-grow-1">
                                    <h5 class="h6 mb-0"><a class="text-black" href="{{route('blog-detail',$latestarticle->id)}}" wire:navigate>{{$latestarticle->title}}</a></h5>
                                    <small>{{\Carbon\Carbon::parse($latestarticle->created_at)->format('F d, Y')}}</small>
                                </div>
                            </li>
                        </ul>
                        @endforeach
